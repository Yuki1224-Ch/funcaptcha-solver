...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Roblox Account Checker with Funcaptcha Solver Integration"
    )
    parser.add_argument(
        "-c", "--combo",
        default=config.COMBO_FILE,
        help=f"Combo file path (default: {config.COMBO_FILE})",
    )
    parser.add_argument(
        "-p", "--proxies",
        default=config.PROXY_FILE,
        help=f"Proxy file path (default: {config.PROXY_FILE})",
    )
    parser.add_argument(
        "-t", "--threads",
        type=int,
    ...
    parser.add_argument(
        "--captcha-url",
        default=config.CAPTCHA_SOLVER_URL,
        help=f"Captcha solver URL (default: {config.CAPTCHA_SOLVER_URL})",
    )
    parser.add_argument(
        "--use-proxies",
        action="store_true",
        default=config.PROXY_ENABLED,
    ...
    parser.add_argument(
        "--delay",
        type=float,
        default=config.CHECK_DELAY,
        help=f"Delay between checks in seconds (default: {config.CHECK_DELAY})",
    )
    ...
    if args.delay:
        config.CHECK_DELAY = args.delay

    # Create the checker and start
    checker = RobloxChecker(
        combo_file=args.combo,
        proxy_file=args.proxies,
        threads=args.threads,
        captcha_solver_url=args.captcha_url,
